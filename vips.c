...
vips_type_find_save_go(int imgtype) {
  ...
  case (PDF):
    return vips_type_find("VipsOperation", "pdfsave_buffer");
  }
...
#else
  vips_error("vips_pdfload_go", "Loading PDF is not supported (libvips 8.3+ reuired)");
  return 1;
#endif
}
...
int
vips_pdfsave_go(VipsImage *in, void **buf, size_t *len) {
    vips_error("vips_pdfsave_go", "Saving PDF is not supported (libvips 8.6+ reuired)");
    return 1;
}
